...
def main(rank_jsonl, task, top_k=1):
    
    query_infos, base_infos = query_base('test', task) #TODO
    ...
    save_file_path = f"/data2/datasets/drivelm/similarity_rank/re_prompt/241103/test_ssim_gp_top_{top_k}_no_img.jsonl"
    save_jsonl(save_file_path, rec_list)
    
    
def main3(rank_jsonl, task, top_k=1):
    ...
    for rank in tqdm(read_jsonl(rank_jsonl)):
        ...
        for base_path in base_paths:
            v_base, q_base, a_base, _ = extract_vqa(base_infos, base_path)
            history.append(['<image>'+q_base, a_base])
            images.append(base_path)
    ...
    save_file_path = f"/data2/datasets/drivelm/similarity_rank/re_prompt/241103/test_ssim_gp_top_{top_k}_all_img.jsonl"
    save_jsonl(save_file_path, rec_list)
...
def main_4_ft(rank_jsonl, task, top_k=1):
    ...
    rec_list = []
    for rank in tqdm(read_jsonl(rank_jsonl_test_vt)):
        query_path = rank['query']
        base_paths = [rank['topk'][i]['base'] for i in range(top_k)]
    ...
    save_file_path = f"/data2/datasets/drivelm/similarity_rank/bert/for_ft/test_vt_gp_top_{top_k}_img.jsonl"
    save_jsonl(save_file_path, rec_list)
    
    
    ### ----------
    
    query_infos, base_infos = query_base('train', task) #TODO
    
    rec_list = []
    for rank in tqdm(read_jsonl(rank_jsonl_train_v)):
        query_path = rank['query']
        base_paths = [rank['topk'][i]['base'] for i in range(top_k)]
    ...
    save_file_path = f"/data2/datasets/drivelm/similarity_rank/bert/for_ft/train_v_gp_top_{top_k}_img.jsonl"
    save_jsonl(save_file_path, rec_list)
    
    query_infos, base_infos = query_base('val', task) #TODO
    
    rec_list = []
    for rank in tqdm(read_jsonl(rank_jsonl_val_t)):
        query_path = rank['query']
        base_paths = [rank['topk'][i]['base'] for i in range(top_k)]
    ...
    save_file_path = f"/data2/datasets/drivelm/similarity_rank/bert/for_ft/val_t_gp_top_{top_k}_img.jsonl"
    save_jsonl(save_file_path, rec_list)
