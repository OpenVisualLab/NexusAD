...
def build_with_no_image(subset, task, rank_jsonl, save_file_path, preds, top_k=1):
    ...
    if task == "general_perception":
        if top_k == 0:
            ...
            new_query = guided_prompt_1 + '<image>' + ori_gp_prompt
    elif task == "driving_suggestion":
        if top_k == 0:
            new_query = '<image>' + ori_ds_prompt
        else:
            new_query = guided_prompt_1 + '<image>' + ori_ds_prompt
    else:
        print(task)
    ...
    for rank in tqdm(read_jsonl(rank_jsonl)):
        ...
        res_q, pred_q = find_the_info(query_path, pred_test)
        ...
        for base_path in base_paths:
            res, pred = find_the_info(base_path, pred_train+pred_val)
            history.append([guided_prompt_0, res])
...
def main_4_ft(top_k=1):
    ...
    rank_jsonl_test_vt = f"/data2/datasets/drivelm/similarity_rank/inetrnvl/test_t_v_top_k.jsonl"
    
    
    # save_file_path = "/data2/datasets/drivelm/similarity_rank/241110_rank_for_infer2/test_vt_gp_v1_0.jsonl"
    # build_with_no_image('test', task, rank_jsonl_test_vt, save_file_path, [pred_test_gp, pred_train_gp, pred_val_gp], top_k=0)
    
    # save_file_path = "/data2/datasets/drivelm/similarity_rank/241110_rank_for_infer2/test_vt_gp_v1_1.jsonl"
    # build_with_no_image('test', task, rank_jsonl_test_vt, save_file_path, [pred_test_gp, pred_train_gp, pred_val_gp], top_k=1)
    
    save_file_path = f"/data2/datasets/drivelm/similarity_rank/00_241115_rank/test_vt_gp_v1_2_{mode}.jsonl"
    build_with_no_image('test', task, rank_jsonl_test_vt, save_file_path, [pred_test_gp, pred_train_gp, pred_val_gp], top_k=2)
    
    # save_file_path = "/data2/datasets/drivelm/similarity_rank/241110_rank_for_infer2/test_vt_gp_v1_3.jsonl"
    # build_with_no_image('test', task, rank_jsonl_test_vt, save_file_path, [pred_test_gp, pred_train_gp, pred_val_gp], top_k=3)
    
    # -----  ds -----
    task = tasks[2]
    
    # save_file_path = "/data2/datasets/drivelm/similarity_rank/241110_rank_for_infer2/test_vt_ds_v1_0.jsonl"
    # build_with_no_image('test', task, rank_jsonl_test_vt, save_file_path, [pred_test_ds, pred_train_ds, pred_val_ds], top_k=0)
    
    # save_file_path = "/data2/datasets/drivelm/similarity_rank/241110_rank_for_infer2/test_vt_ds_v1_1.jsonl"
    # build_with_no_image('test', task, rank_jsonl_test_vt, save_file_path, [pred_test_ds, pred_train_ds, pred_val_ds], top_k=1)
    
    save_file_path = f"/data2/datasets/drivelm/similarity_rank/00_241115_rank/test_vt_ds_v1_2_{mode}.jsonl"
    build_with_no_image('test', task, rank_jsonl_test_vt, save_file_path, [pred_test_ds, pred_train_ds, pred_val_ds], top_k=2)
    
    # save_file_path = "/data2/datasets/drivelm/similarity_rank/241110_rank_for_infer2/test_vt_ds_v1_3.jsonl"
    # build_with_no_image('test', task, rank_jsonl_test_vt, save_file_path, [pred_test_ds, pred_train_ds, pred_val_ds], top_k=3)
