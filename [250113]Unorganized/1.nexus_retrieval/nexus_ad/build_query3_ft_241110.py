...
def build_with_no_image(subset, task, rank_jsonl, save_file_path, preds, top_k=1):
    ...
    guided_prompt_2 = " You will be given an image along with the current response to improve. Your task is to analyze the provided response in combination with the given image, identify the issues, and generate a higher-quality version of the response."
    ...
    if task == "general_perception":
        if top_k == 0:
            ...
            new_query = guided_prompt_2 + '<image>' + ori_gp_prompt
    elif task == "driving_suggestion":
        if top_k == 0:
            new_query = '<image> ' + ori_ds_prompt
        else:
            new_query = guided_prompt_2 + '<image>' + ori_ds_prompt
    else:
        print(task)
...
def main_4_ft(top_k=1):
    ...
    save_file_path = "/data2/datasets/drivelm/similarity_rank/241110_rank_for_ft2/test_vt_gp_v1_3.jsonl"
    build_with_no_image('test', task, rank_jsonl_test_vt, save_file_path, [pred_test_gp, pred_train_gp, pred_val_gp], top_k=3)
    
    # save_file_path = "/data2/datasets/drivelm/similarity_rank/241110_rank_for_ft2/train_v_gp_v1_2.jsonl"
    # build_with_no_image('train', task, rank_jsonl_train_v, save_file_path, [pred_test_gp, pred_train_gp, pred_val_gp], top_k=2)
    
    # save_file_path = "/data2/datasets/drivelm/similarity_rank/241110_rank_for_ft2/val_t_gp_v1_2.jsonl"
    ...
    save_file_path = "/data2/datasets/drivelm/similarity_rank/241110_rank_for_ft2/test_vt_ds_v1_3.jsonl"
    build_with_no_image('test', task, rank_jsonl_test_vt, save_file_path, [pred_test_ds, pred_train_ds, pred_val_ds], top_k=3)
    
    # save_file_path = "/data2/datasets/drivelm/similarity_rank/241110_rank_for_ft2/train_v_ds_v1_2.jsonl"
    # build_with_no_image('train', task, rank_jsonl_train_v, save_file_path, [pred_test_ds, pred_train_ds, pred_val_ds], top_k=2)
    
    # save_file_path = "/data2/datasets/drivelm/similarity_rank/241110_rank_for_ft2/val_t_ds_v1_2.jsonl"
