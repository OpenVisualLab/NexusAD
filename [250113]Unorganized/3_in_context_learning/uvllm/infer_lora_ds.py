...
from vllm_vl import init_model, run_model
...
def prepare_batch_data(data_batch, data_root):    
    ...
    full_image_paths = [data['images'] for data in data_batch]
    histories = [data['history'] for data in data_batch]
    
    return questions, full_image_paths, histories
...
def main(data_root, model_dir, json_path, out_json_path, batch_size=1, tqdm_position=0): #TODO
    ...
    model, template = init_model(model_id_or_path=model_dir)
    ...
    with open(out_json_path, 'a') as outf, tqdm(total=total_samples, position=tqdm_position, leave=True, desc='Sample Progress') as pbar:
        for batch in batched_lines:
            ...
            questions, full_image_paths, histories = prepare_batch_data(data_batch, data_root)
            
            responses = [run_model(model, template, questions[0], full_image_paths[0], histories[0])]
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--model_dir', type=str, default='/data2/datasets/drivelm/output2/internvl2-8b/v3-20241024-023750_structure_gp/checkpoint-2316-merged', help='Directory of model weights')
    
    # parser.add_argument('--json_path', type=str, default='/data2/datasets/drivelm/similarity_rank/aaaa_for_ft/test_vt_gp_top_2_img.jsonl', help='Path to the input JSON file')
    # parser.add_argument('--out_json_path', type=str, default='/data2/datasets/drivelm/241110/test_icl_gp_struct.jsonl', help='Path to the output JSON file')
    # parser.add_argument('--tqdm_position', type=int, default=0, help='Position of tqdm progress bar')

    # args = parser.parse_args()
    # main(args.data_root, args.model_dir, args.json_path, args.out_json_path, args.batch_size, args.tqdm_position)


    parser = argparse.ArgumentParser(description='Process a subset and task of the CODA dataset using a specified model.')
    parser.add_argument('--data_root', type=str, default='/data1/nemo/projects/240922_AD/datasets/images', help='Root directory of datasets')
    parser.add_argument('--batch_size', type=int, default=1, help='Batch size')
    # parser.add_argument('--model_dir', type=str, default='/data2/datasets/drivelm/output3/241103_gp_icl/checkpoint-1737-merged', help='Directory of model weights')
    parser.add_argument('--model_dir', type=str, default='/data2/datasets/drivelm/output2/internvl2-8b/v0-20241023-225447_ori_gp/checkpoint-2316-merged', help='Directory of model weights')
    
    parser.add_argument('--json_path', type=str, default='/data2/datasets/drivelm/similarity_rank/aaaa_for_ft/test_vt_gp_top_2_v1.jsonl', help='Path to the input JSON file')
    parser.add_argument('--out_json_path', type=str, default='/data2/datasets/drivelm/241110/test_ft_gp_v1.jsonl', help='Path to the output JSON file')
    parser.add_argument('--tqdm_position', type=int, default=0, help='Position of tqdm progress bar')
