...
def loop_test(root, dst):
    ...
    for scene_id in tqdm(test_file.keys(), desc="Converting"):
...
if __name__ == '__main__':
    # data_root = "/data1/nemo/projects/241009_DriveLM/drivelm_data_demo"
    
    # root = os.path.join(data_root, "test.json")
    # dst = os.path.join(data_root, "test_eval.json")
    # loop_test(root, dst)
    
    # 创建 ArgumentParser 对象
    parser = argparse.ArgumentParser(description="Convert data from one JSON file to another")
    
    # 添加参数
    parser.add_argument("input_path", type=str, help="Path to the input JSON file")
    parser.add_argument("output_path", type=str, help="Path to save the converted JSON file")
    
    # 解析命令行参数
    args = parser.parse_args()
    
    # 调用 loop_test 函数
    loop_test(args.input_path, args.output_path)
