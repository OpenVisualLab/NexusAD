...
def stage_1_eval(reference_path, prediction_path, save_path, api_info):
    
    os.makedirs(save_path, exist_ok=True)
    json_list = sorted([each for each in os.listdir(reference_path) if each.endswith(".json")])
    answers = [json.loads(q) for q in open(os.path.expanduser(prediction_path), "r", encoding='utf-8')]
    ...
    for idx, json_name in tqdm(enumerate(json_list)):
        ...
        message["prediction"] = answers[idx][api_info['eval_key']]
    ...
    for idx, json_name in tqdm(enumerate(json_list)):
        ...
        with open(os.path.join(save_path, txt_name), "w", encoding='utf-8') as f:
            f.write(output)
