...
class RegionEval(object):
    ...
    def __init__(self, reference_path, prediction_path, api_info):
        self.k = api_info["eval_key"]
    ...
    def load_predict(self, prediction_path):
        ...
        for line_data in answers:
            ...
            self.predict_data[new_label_name].append({
                # "question": line_data["query"], 
                "answer": line_data[self.k]
...
def stage_3_eval(reference_path, prediction_path, save_path, api_info):
    
    os.makedirs(save_path, exist_ok=True)
    os.makedirs(os.path.join(save_path, "gpt_result"), exist_ok=True)
    region_eval = RegionEval(reference_path, prediction_path, api_info)
    batcher = GPTBatcher(
        api_key=api_info["api_key"], 
        model_name=api_info["model_name"], 
        num_workers=api_info["num_workers"],
        api_base_url=api_info["api_base_url"])
    ...
    for label_name in tqdm(label_info.keys()):
        ...
        for idx, txt_name in tqdm(enumerate(txt_names)):
            ...
            with open(os.path.join(save_path, "gpt_result", txt_name), "w") as f:
                f.write(output)

        with open(os.path.join(save_path, f"{label_name}.txt"), "w") as fp:
            if len(cls_score) == 0:
    ...
    with open(os.path.join(save_path, "all_score.txt"), "w", encoding='utf-8') as f:
        f.write(f"Stage3_score: {sum(all_score)/len(all_score)}")

if __name__ == "__main__":
    prediction_path = "/data2/datasets/drivelm/output2/nexusad/241024_ori_vl_8b/ori_vl_8b_test_region_perception.jsonl"
    api_info = {
        "num_workers":50,
    ...
    label_dir = "/data1/nemo/projects/240922_AD/datasets/labels/CODA-LM"
    subset = "Test" 
    reference_path = os.path.join(label_dir, subset)
    save_path = prediction_path.split('.')[0] 
    
    stage_3_eval(reference_path, prediction_path, save_path, api_info)